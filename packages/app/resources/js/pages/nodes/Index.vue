<script setup lang="ts">
import { ref, onMounted, computed } from 'vue';
import { Head, Link, router } from '@inertiajs/vue3';
import Heading from '@/components/Heading.vue';
import { Server, Loader2, AlertCircle, Trash2, Pencil, Eye } from 'lucide-vue-next';
import { Button, Badge, Table, TableHeader, TableBody, TableRow, TableHead, TableCell, TableEmpty } from '@hardimpactdev/craft-ui';

interface Node {
    id: number;
    name: string;
    host: string;
    user: string;
    port: number;
    is_local: boolean;
    status: string | null;
    provisioning_step: number | null;
    provisioning_total_steps: number | null;
    last_connected_at: string | null;
}

const props = defineProps<{
    nodes: Node[];
    hasLocalNode: boolean;
}>();

// TLD tracking
const tldMap = ref<Map<number, string>>(new Map());
const conflictingTlds = ref<Set<string>>(new Set());

const loadTld = async (nodeId: number) => {
    try {
        const response = await fetch(`/nodes/${nodeId}/config`);
        const result = await response.json();

        if (result.success && result.data) {
            const tld = result.data.tld || 'test';
            tldMap.value.set(nodeId, tld);
            checkForConflicts();
        }
    } catch (error) {
        console.error(`Failed to load TLD for node ${nodeId}:`, error);
    }
};

const checkForConflicts = () => {
    const tldCounts = new Map<string, number>();

    tldMap.value.forEach((tld) => {
        tldCounts.set(tld, (tldCounts.get(tld) || 0) + 1);
    });

    const conflicts = new Set<string>();
    tldCounts.forEach((count, tld) => {
        if (count > 1) {
            conflicts.add(tld);
        }
    });
    conflictingTlds.value = conflicts;
};

const getTld = (nodeId: number) => tldMap.value.get(nodeId);
const isConflict = (nodeId: number) => {
    const tld = tldMap.value.get(nodeId);
    return tld ? conflictingTlds.value.has(tld) : false;
};

const getConflictCount = (nodeId: number) => {
    const tld = tldMap.value.get(nodeId);
    if (!tld) return 0;

    let count = 0;
    tldMap.value.forEach((t) => {
        if (t === tld) count++;
    });
    return count;
};

const deleteNode = (node: Node) => {
    if (confirm('Are you sure you want to delete this node?')) {
        router.delete(`/nodes/${node.id}`);
    }
};

const formatLastConnected = (dateString: string | null) => {
    if (!dateString) return 'Never';

    const date = new Date(dateString);
    const now = new Date();
    const diffMs = now.getTime() - date.getTime();
    const diffMins = Math.floor(diffMs / 60000);
    const diffHours = Math.floor(diffMs / 3600000);
    const diffDays = Math.floor(diffMs / 86400000);

    if (diffMins < 1) return 'Just now';
    if (diffMins < 60) return `${diffMins} minute${diffMins !== 1 ? 's' : ''} ago`;
    if (diffHours < 24) return `${diffHours} hour${diffHours !== 1 ? 's' : ''} ago`;
    if (diffDays < 30) return `${diffDays} day${diffDays !== 1 ? 's' : ''} ago`;
    return date.toLocaleDateString();
};

onMounted(() => {
    props.nodes.forEach((node) => loadTld(node.id));
});
</script>

<template>
    <Head title="Nodes" />

    <div>
        <div class="flex justify-between items-center mb-8">
            <Heading title="Nodes" />
            <Button v-if="$page.props.multi_node" as-child>
                <Link href="/nodes/create">
                    Add Node
                </Link>
            </Button>
        </div>

        <Table>
            <TableHeader>
                <TableRow>
                    <TableHead>Name</TableHead>
                    <TableHead>Host</TableHead>
                    <TableHead>Status</TableHead>
                    <TableHead>Last connected</TableHead>
                    <TableHead class="text-right">Actions</TableHead>
                </TableRow>
            </TableHeader>
            <TableBody>
                <TableRow v-for="node in nodes" :key="node.id">
                    <TableCell class="whitespace-nowrap">
                        <div class="flex items-center">
                            <span class="font-medium text-foreground">{{ node.name }}</span>
                            <!-- TLD Badge -->
                            <Badge
                                v-if="getTld(node.id)"
                                class="ml-2 font-mono"
                                :variant="isConflict(node.id) ? 'destructive' : 'secondary'"
                                :title="
                                    isConflict(node.id)
                                        ? `Warning: ${getConflictCount(node.id)} nodes use .${getTld(node.id)} - this may cause DNS conflicts`
                                        : ''
                                "
                            >
                                .{{ getTld(node.id) }}
                            </Badge>
                            <Badge v-if="node.is_local" class="ml-2 bg-lime-400/10 text-lime-300 border-lime-400/20">
                                Local
                            </Badge>
                        </div>
                    </TableCell>
                    <TableCell class="whitespace-nowrap text-muted-foreground">
                        <template v-if="node.is_local">localhost</template>
                        <template v-else
                            >{{ node.user }}@{{ node.host }}:{{
                                node.port
                            }}</template
                        >
                    </TableCell>
                    <TableCell class="whitespace-nowrap">
                        <!-- Provisioning Status -->
                        <span
                            v-if="node.status === 'provisioning'"
                            class="inline-flex items-center text-sm text-blue-400"
                        >
                            <Loader2 class="w-3.5 h-3.5 mr-2 animate-spin" />
                            Provisioning ({{ node.provisioning_step ?? 0 }}/{{
                                node.provisioning_total_steps ?? 14
                            }})
                        </span>
                        <!-- Error Status -->
                        <span
                            v-else-if="node.status === 'error'"
                            class="inline-flex items-center text-sm text-red-400"
                        >
                            <AlertCircle class="w-3.5 h-3.5 mr-2" />
                            Error
                        </span>
                        <!-- Active Status -->
                        <span v-else class="inline-flex items-center text-sm text-lime-400">
                            <span class="w-2 h-2 rounded-full bg-lime-400 mr-2"></span>
                            Active
                        </span>
                    </TableCell>
                    <TableCell class="whitespace-nowrap text-muted-foreground">
                        {{ formatLastConnected(node.last_connected_at) }}
                    </TableCell>
                    <TableCell class="whitespace-nowrap text-right">
                        <div class="flex items-center justify-end gap-1">
                            <Button as-child variant="ghost" size="icon-sm">
                                <Link
                                    :href="`/nodes/${node.id}`"
                                    title="View"
                                >
                                    <Eye class="w-4 h-4" />
                                </Link>
                            </Button>
                            <Button v-if="$page.props.multi_node" as-child variant="ghost" size="icon-sm">
                                <Link
                                    :href="`/nodes/${node.id}/edit`"
                                    title="Edit"
                                >
                                    <Pencil class="w-4 h-4" />
                                </Link>
                            </Button>
                            <Button
                                v-if="$page.props.multi_node"
                                @click="deleteNode(node)"
                                variant="ghost"
                                size="icon-sm"
                                class="text-muted-foreground hover:text-red-400"
                                title="Delete"
                            >
                                <Trash2 class="w-4 h-4" />
                            </Button>
                        </div>
                    </TableCell>
                </TableRow>
                <!-- Empty State -->
                <TableEmpty v-if="nodes.length === 0" :colspan="5">
                    <Server class="w-12 h-12 mx-auto text-zinc-600 mb-4" />
                    <p class="text-zinc-400 mb-2">No nodes configured yet</p>
                    <p class="text-sm text-zinc-500">
                        Add a local or remote node to get started
                    </p>
                </TableEmpty>
            </TableBody>
        </Table>
    </div>
</template>
