...
// Whether this is an external node viewed from desktop
const isExternalNode = computed(() => !props.node.is_local && props.remoteApiUrl);
...
// Instance info sync (for external nodes)
...
async function runDoctor() {
    ...
    try {
        const response = await fetch(`/nodes/${props.node.id}/doctor`);
...
async function fixIssue(checkKey: string) {
    ...
    try {
        const response = await fetch(
            `/nodes/${props.node.id}/doctor/fix/${checkKey}`,
            {
                method: "POST",
                headers: {
...
async function loadInstanceInfo() {
    if (!isExternalNode.value || !props.remoteApiUrl) return;
    ...
    try {
        ...
        if (result.success && result.data) {
            nodeForm.name = result.data.name;
        } else {
            instanceInfoError.value = result.error || "Failed to load instance info";
        }
    } catch (error) {
        instanceInfoError.value = "Failed to connect to remote node";
    } finally {
...
async function saveRemoteName() {
    ...
    try {
        const response = await fetch(`${props.remoteApiUrl}/instance-info`, {
            ...
            body: JSON.stringify({ name: nodeForm.name }),
        });
...
async function loadConfig() {
    ...
    try {
        const response = await fetch(`/api/nodes/${props.node.id}/config`);
        const result = await response.json();
...
async function saveEnvSettings() {
    if (isExternalNode.value) {
        await saveRemoteName();
    }
    nodeForm.post(`/nodes/${props.node.id}/settings`);
}
...
async function loadDirectories(path: string) {
    ...
    try {
        const response = await fetch(
            `/nodes/${props.node.id}/browse-directories?path=${encodeURIComponent(path)}`,
        );
        const result = await response.json();
...
async function saveConfig() {
    ...
    try {
        const response = await fetch(`/nodes/${props.node.id}/config`, {
            method: "POST",
...
function deleteNode() {
    if (deleteConfirmName.value !== props.node.name) {
        return;
    }

    router.delete(`/nodes/${props.node.id}`, {
        onSuccess: () => {
...
const externalAccessForm = useForm({
    external_access: props.node.external_access ?? false,
    external_host: props.node.external_host ?? "",
});

const saveExternalAccess = () => {
    externalAccessForm.post(`/nodes/${props.node.id}/configuration/external-access`);
};
...
<template>
    <Head :title="`Configuration - ${node.name}`" />
        ...
        <aside class="sticky top-0 h-screen w-52 shrink-0 bg-zinc-950">
            <div class="flex h-full flex-col">
                ...
                <div class="p-6">
                    ...
                    <p class="text-sm text-zinc-400">{{ node.name }}</p>
                </div>
        ...
        <main class="flex-1 p-6 lg:p-8 overflow-auto">
            <div class="max-w-2xl mx-auto">
                ...
                <div v-if="activeTab === 'configuration'">
                    ...
                    <form @submit.prevent="saveEnvSettings">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-start gap-4 py-6">
                            <div class="sm:w-1/2 space-y-1">
                                <Label class="text-sm font-medium text-white">Node Name</Label>
                                <p class="text-xs text-zinc-500">
                                    Display name for this node.
                                    <template v-if="isExternalNode">
                                        <br /><span class="text-zinc-600">Changes will be synced to the remote node.</span>
                                    </template>
                                </p>
                            </div>
                            <div class="sm:w-1/2">
                                ...
                                <p v-if="nodeForm.errors.name" class="mt-2 text-sm text-red-400">
                                    {{ nodeForm.errors.name }}
                                </p>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-start gap-4 py-6">
                            ...
                            <div class="sm:w-1/2">
                                <Select v-model="nodeForm.editor_scheme">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-start gap-4 py-6">
                                ...
                                <div class="sm:w-1/2 space-y-3">
                                    <div class="space-y-1.5">
                                        ...
                                        <Input v-model="nodeForm.host" type="text" class="font-mono" />
                                        <p v-if="nodeForm.errors.host" class="text-sm text-red-400">
                                            {{ nodeForm.errors.host }}
                                        </p>
                                    </div>
                                    <div class="grid grid-cols-2 gap-3">
                                        <div class="space-y-1.5">
                                            <Label class="text-xs text-zinc-500">User</Label>
                                            <Input v-model="nodeForm.user" type="text" class="font-mono" />
                                            <p v-if="nodeForm.errors.user" class="text-sm text-red-400">
                                                {{ nodeForm.errors.user }}
                                            </p>
                                        </div>
                                        <div class="space-y-1.5">
                                            <Label class="text-xs text-zinc-500">Port</Label>
                                            <Input v-model="nodeForm.port" type="number" class="font-mono" />
                                            <p v-if="nodeForm.errors.port" class="text-sm text-red-400">
                                                {{ nodeForm.errors.port }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="flex justify-end py-6">
                            <Button
                                type="submit"
                                :disabled="nodeForm.processing || nameSaving"
                                class="bg-lime-500 hover:bg-lime-600 text-zinc-950"
                            >
                                {{ nodeForm.processing || nameSaving ? "Saving..." : "Save Node" }}
                            </Button>
                        </div>
                    </form>
                    ...
                    <template v-if="multiNode">
                        <Separator class="bg-zinc-800 my-4" />

                        <div class="py-6">
                            <div class="mb-6">
                                <Label class="text-sm font-medium text-white">SSH Keys</Label>
                                <p class="text-xs text-zinc-500 mt-1">Manage SSH keys for node provisioning.</p>
                            </div>
                ...
                <div v-if="activeTab === 'advanced'">
                    ...
                    <div v-if="multiNode" class="space-y-6 mb-8">
                        <!-- Notifications -->
                        <div class="flex flex-col sm:flex-row sm:items-center gap-4 py-6">
                            <div class="sm:w-1/2 space-y-1">
                        ...
                        <div class="space-y-4">
                            <div class="rounded-lg border border-red-500/30 bg-red-500/5 p-4">
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    <div>
                                        <p class="text-sm font-medium text-white">Delete Node</p>
                                        <p class="text-xs text-zinc-500">Permanently delete this node.</p>
                                    </div>
                                    <Button
                                    ...
                                    <div class="flex items-start gap-3 p-3 bg-red-900/20 rounded-lg">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-red-400/80">
                                                Type <strong class="text-red-300">{{ node.name }}</strong> to confirm.
                                            </p>
                                        </div>
                                    </div>
                                    <Input
