...
final class CloudflareCachePurgeCommand extends Command
{
    use WithJsonOutput;
    ...
    protected $description = 'Purge Cloudflare cache for a zone (runs on gateway)';
    ...
    public function handle(CloudflareService $cloudflare): int
    {
        ...
        $success = $cloudflare->purgeCache($zoneId);

        if (! $success) {
            return $this->outputJsonError('Failed to purge cache.');
        }
