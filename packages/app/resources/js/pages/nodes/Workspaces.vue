...
const props = defineProps<{
    node: Node;
    editor: Editor;
    remoteApiUrl: string | null; // Direct API URL for remote nodes (bypasses NativePHP)
}>();
...
const getApiUrl = (path: string) => {
    ...
    return `/api/nodes/${props.node.id}${path}`;
};
...
const openInEditor = (workspace: Workspace) => {
    ...
    let url: string;
    if (props.node.external_access || !props.node.is_local) {
        // Use SSH remote URL for external access or remote nodes
        const user = props.node.user;
        const host = props.node.external_access && props.node.external_host
            ? props.node.external_host
            : props.node.host;
...
<template>
    <Head :title="`Workspaces - ${node.name}`" />
        ...
        <header class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8">
            ...
            <div class="flex items-center gap-2">
                <Button as-child size="sm" class="bg-lime-500 hover:bg-lime-600 text-zinc-950">
                    <Link :href="`/nodes/${node.id}/workspaces/create`">
            ...
            <div>
                ...
                >
                    ...
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <Link
                            :href="`/nodes/${node.id}/workspaces/${workspace.name}`"
