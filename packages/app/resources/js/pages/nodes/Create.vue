...
// Node type toggle
const nodeType = ref<'local' | 'remote'>(props.hasLocalNode ? 'remote' : 'local');
...
<template>
    ...
    <div>
        ...
        <Heading title="Add Node" />
        <p class="text-zinc-400 mb-8 mt-2">
            Add a local or remote node to manage development projects.
        </p>
        ...
        <div class="mb-8">
            <div class="inline-flex rounded-lg bg-zinc-800 p-1">
                <button
                    type="button"
                    @click="nodeType = 'local'"
                    :disabled="hasLocalNode"
                ...
                >
                    Local
                    <span v-if="hasLocalNode" class="ml-1 text-xs">(configured)</span>
                </button>
                <button
                    type="button"
                    @click="nodeType = 'remote'"
                    class="px-4 py-1.5 text-sm font-medium rounded-md transition-colors"
                    :class="
                        nodeType === 'remote'
                            ? 'bg-zinc-600 text-white'
        ...
        <!-- Local Node Form -->
        <form v-if="nodeType === 'local'" @submit.prevent="submitLocal" class="max-w-lg">
            <div class="space-y-6">
                <div>
