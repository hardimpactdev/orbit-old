...
async function checkRepoExists(org: string, name: string) {
    ...
    try {
        const response = await fetch(`/nodes/${props.node.id}/github-repo-exists`, {
            method: 'POST',
            headers: {
...
const fetchTemplateDefaults = async (template: string) => {
    ...
    try {
        const response = await fetch(`/nodes/${props.node.id}/template-defaults`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN':
...
onMounted(async () => {
    ...
    try {
        const [userResult, orgsResult] = await Promise.all([
            fetch(`/nodes/${props.node.id}/github-user`).then((r) => r.json()),
            fetch(`/nodes/${props.node.id}/github-orgs`).then((r) => r.json()),
        ]);
        ...
        try {
            const userResult = await fetch(`/nodes/${props.node.id}/github-user`).then((r) => r.json());
            if (userResult.success && userResult.user) {
                githubUser.value = userResult.user.toLowerCase();
                selectedOrg.value = githubUser.value;
            }
...
<template>
    ...
    <div>
        <div class="mb-8">
            ...
            <p class="text-zinc-400 mt-1">Create a new project in {{ node.name }}</p>
        </div>
