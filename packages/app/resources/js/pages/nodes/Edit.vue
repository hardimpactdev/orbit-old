...
const form = useForm({
    ...
    is_local: props.node.is_local,
    external_access: props.node.external_access,
...
const conflictingServers = computed(() => {
    ...
    return Object.entries(otherServerTlds.value)
        .filter(([id, serverTld]) => serverTld === currentTld && parseInt(id) !== props.node.id)
        .map(([id]) => parseInt(id));
});
...
const loadOtherServerTlds = async () => {
    try {
        const response = await fetch('/api/nodes/tlds');
        const result = await response.json();
        if (result.success) {
                ...
                <div class="pt-6 border-t border-border">
                    ...
                    <div>
                        ...
                        <p v-if="hasConflict" class="mt-2 text-sm text-amber-400 flex items-center">
                            ...
                            Another node is using .{{ tldPreview }} - this may cause DNS
                            conflicts
                        </p>
                    </div>
                </div>
