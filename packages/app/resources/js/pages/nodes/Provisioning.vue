...
const currentStep = ref(props.node.provisioning_step ?? 0);
const totalSteps = ref(props.node.provisioning_total_steps ?? 17);
const log = ref<LogEntry[]>(props.node.provisioning_log ?? []);
const error = ref(props.node.provisioning_error);
...
const checklistItems = computed(() => {
    return props.node.is_local ? localChecklistItems : remoteChecklistItems;
});
...
async function startProvisioning() {
    ...
    try {
        await fetch(`/provision/${props.node.id}/run`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN':
...
async function pollStatus() {
    try {
        const response = await fetch(`/provision/${props.node.id}/status`);
...
onMounted(() => {
    if (props.node.status === 'provisioning') {
        startPolling();
...
<template>
    <Head :title="`${node.name} - Provisioning`" />

    <div class="p-6 max-w-2xl">
        <div class="mb-6">
            <Link href="/nodes" class="text-blue-600 hover:text-blue-800 flex items-center">
        ...
        <div class="flex justify-between items-start mb-6">
            <div>
                <h2 class="text-2xl font-bold text-gray-800 dark:text-white">{{ node.name }}</h2>
                <p class="text-gray-500 dark:text-gray-400">{{ node.host }}</p>
            </div>
        </div>
            ...
            <div class="flex items-center mb-4">
                ...
                <div>
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white">
                        {{ isError ? 'Provisioning Failed' : 'Setting up Node' }}
                    </h3>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-4">
                {{ node.is_local ? "What's being installed (Mac)" : "What's being installed" }}
            </h3>
