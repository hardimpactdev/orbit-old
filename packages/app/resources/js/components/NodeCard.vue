...
<template>
    <div class="border border-border rounded-lg p-4 hover:border-muted-foreground/30 transition-colors">
        <div class="flex justify-between items-start mb-3">
            <div>
                <h3 class="font-semibold text-foreground">{{ node.name }}</h3>
                <p class="text-sm text-muted-foreground">
                    <template v-if="node.is_local">Local</template>
                    <template v-else>{{ node.user }}@{{ node.host }}</template>
                </p>
            </div>
            <div class="flex items-center space-x-2">
                <Badge v-if="node.is_default" class="bg-blue-500/15 text-blue-400 border-blue-400/20">Default</Badge>
