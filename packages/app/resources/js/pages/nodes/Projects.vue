...
function handleApiError(error: string | undefined, context: string) {
    ...
    if (errorMsg.includes('Connection error')) {
        toast.error('Connection Error', {
            description: 'Could not connect to the node. Check if Orbit is running.',
        });
    } else {
...
function openInEditor(path: string) {
    let url;
    if (props.node.is_local) {
        url = `${props.editor.scheme}://file${path}`;
    } else {
        const sshHost = `${props.node.user}@${props.node.host}`;
    ...
    <div>
        ...
        <header class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8">
            <div>
                ...
                <p class="text-sm text-zinc-500 mt-1">Projects in {{ node.name }}</p>
            </div>
            <div class="flex items-center gap-2">
                <Button as-child size="sm" class="bg-lime-500 hover:bg-lime-600 text-zinc-950">
                    <Link :href="`/nodes/${node.id}/projects/create`">
            ...
            <div>
                ...
                <p class="text-zinc-400 text-sm mt-1">
                    <span v-if="!isProvisioningConfigured">
                        Reverb is not configured for this node. Status updates for
        ...
        >
            ...
            <p class="text-zinc-400">Add project directories in the node configuration.</p>
        </div>
